...
/* This function samples the mass model with particles.
 * The main job of the code will then be to find the initial velocities.
 */
void initialize_particles(void)
{
  ...
  for(n = 0; n < NumPart; n++)
      ...
      if(disp_p <= All.LowerDispLimit)
	{
      ...
      if(typeOfVelocityStructure == 0 || typeOfVelocityStructure == 1 || typeOfVelocityStructure == 3) /* spherical case */
	{
	  ...
	  */
      ...
      else if(typeOfVelocityStructure == 2) /* axisymmetric case, f(E,Lz), with net rotation */
	{
	  double sigmaR = sqrt(disp_r);
	  double sigmaT = sqrt(disp_t);
	  double sigmaP = sqrt(disp_p);
...
{
  if(((struct permutation_data *) a)->rnd < (((struct permutation_data *) b)->rnd))
    return -1;
...
/* this function outputs the rotation curve.
 */
